...
Furthermore, ecDNA elements may reintegrate back into the genome to form HSRs. The inter-cell heterogeneity in copy number of ecDNA as well as the interchangeability between ecDNA and HSR may allow the tumor to adapt to changing environment, e.g. targeted drug application. As a result, understanding the architecture of the focal amplifications is important to gain insights into cancer biology. AmpliconArchitect (AA) is a tool which can reconstruct the structure of focally amplified regions (>10kbp) in a cancer sample using whole genome sequence short paired-end data.

Please check out the **detailed guide** on running AA [available here](https://github.com/AmpliconSuite/AmpliconSuite-pipeline/blob/master/documentation/GUIDE.md) to learn about best practices and see some FAQs.
...
AmpliconArchitect's copy-number optimization supports **Clarabel**, a free, open-source solver. **You no longer need to obtain a Mosek license to run AmpliconArchitect.** AA requests Mosek by default; if Mosek is unavailable or fails during optimization, AA prints a warning and retries with Clarabel. Set `--solver clarabel` to use Clarabel directly without attempting Mosek. In our testing the results are nearly identical between the two solvers, with only negligible differences — the choice only affects runtime, by a few minutes per sample at most.

If you have a Mosek license (free for academic use), place it at `$HOME/mosek/mosek.lic`. Otherwise the default Mosek attempt fails over to Clarabel with no license configuration needed.
...
| `--plotstyle` | STR   | Values: [`small`, `large`, `all_amplicons`]. `large`: large font, `all_amplicons`: display a large number of intervals in a single plot, recommended for visualizing multiple amplicons in CLUSTERED mode. Default: `small`                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `--no_cstats`   | FLAG  | Values: `--no_cstats`  Do not read from or write to the `$AA_DATA_REPO/coverage.stats` file. Set this if trying multiple different values of `--insert_sdevs` or `--pair_support_min`. (default not set)                                                                                                                                                                                                                                                                                                                                                                                                     |
...
The edges in this section correspond to a segment on the reference genome. Sequence edges are a list of non-overlapping segments covering all the intervals in the amplicon. Tab-separated fields:
    ...
    3. EndPosition: Breakpoint vertex correspondong to the coordinate of the last base in the segment.
...
The edges in the section correspond to 2 positions (breakpoint vertices) in the reference genome that are connected together in the amplicon. Tab-separated fields:
    ...
    2. StartPosition->EndPosition: `{POSITION1}->{POSITION2}` where `{POSITION1}` and `{POSITION2}` are the coordinates and strands of the connected breakpoint vertices.
...
### 5. [Intermediate] Copy number segmentation file `{out}_{CHROM}_{START}_{END}_... ..._cnseg.txt`
...
3. Breakpoint graph construction: Construct a breakpoint graph consisting of sequence edges (genomic segments), breakpoint edges (pairs of connected genomic positions) and optionally a source vertex and predict copy counts for all edges.
